...
type SettingDialogueBoxProps = {
  open: boolean;
  onClose: () => void;
  popupVideoPlayerRef: any;
  webcams: any[];
  mics: any[];
  setting: string;
  setSetting: (s: string) => void;
  setSelectedMic: (s: any) => void;
...
const AudioAnalyser = ({ audioTrack }: { audioTrack: MediaStreamTrack }) => {
  ...
  useEffect(() => {
    const audioTrackRef = useRef<MediaStreamTrack | undefined>();
